    ...
    class Class1
    {
        static void Main(String[] args)
        {
            ...
            var bus = Bus.Factory.CreateUsingRabbitMq(sbc =>
            {
                ...
                sbc.ReceiveEndpoint(host, "klientb", ep => ep.Instance(Klient));
            });
            bus.Start();
            Console.WriteLine("Klient B startuje");
            while (true)
            {
                int zamow = 0;
                if (Console.ReadKey().Key == ConsoleKey.Spacebar)
                {
                    ...
                    bus.Publish(new ZlozenieZamowienia() { login = "Klient_B", ilosc = zamow });
                }
            }
            bus.Stop();
        }
